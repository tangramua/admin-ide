...
const _deriveSymmetricKey = salt => crypto.pbkdf2Sync(config.AUTH0_CLIENT_SECRET, salt, 10000, 32, 'sha256');
...
export const encryptSession = session => {
    ...
    const enc = Buffer.concat([cipher.update(JSON.stringify(session) as any), cipher.final()]);
