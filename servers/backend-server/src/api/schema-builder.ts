...
import { buildSchema as buidGithubSchema } from '@adminide-stack/git-api-server';
...
export class GatewaySchemaBuilder {
    ...
    public async build(): Promise<GraphQLSchema> {
        ...
        try {
            ...
            let data = await this.buildRemoteSchemas();
    ...
    private buildRemoteSchemas() {
        return Promise.all(
                [ buidGithubSchema ].map(build => build()),
        );
    }
